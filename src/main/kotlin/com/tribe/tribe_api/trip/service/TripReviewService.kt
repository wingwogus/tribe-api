...
class TripReviewService(
    ...
    private fun createPromptFromTrip(trip: Trip, concept: String?): String {
        ...
        * **A. 전체적인 동선의 효율성:** 여행 콘셉트를 고려하여 동선을 분석하는데 공항은 포함하지 말아야합니다. (예: '뚜벅이 여행' 콘셉트라면 대중교통 중심의 효율적인 동선을, '드라이브 여행'이라면 주차 편의성을 고려)
        * **B. 각 날짜별 일정의 현실성:** '${concept}'에 맞춰 일정의 밀도를 평가합니다. (예: '힐링 여행'이라면 여유로운 일정을, '알찬 여행'이라면 빡빡해도 괜찮다는 점을 감안)
        * **C. 숨겨진 명소 및 맛집 추천:**
            * **가장 중요:** 추천하는 모든 장소와 맛집은 '${concept}'과 **직접적으로 관련된 곳이어야 하고 ${trip.country.koreanName}의 속한 곳이어야 합니다.**
            * **예시:**
        ...
        * **D. 최적화 동선 추천:** 현재 일정들의 동선을 파악하고 이보다 더 효율적인 동선이 있을 시 해당 동선을 추천하고 사용자들의 위시리스트에 있는 곳들을 우선시합니다.
        ---
        ...
        **4. 추천 장소 목록
        ...
        [사용자들의 위시리스트]
        ${trip.wishlistItems.joinToString("\n") { "    - 이름: ${it.place.name}, 주소: ${it.place.address}" }}}
        """.trimIndent()
